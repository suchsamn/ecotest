...
function showPage(id) {
  ...
  if (id === 'chat') { renderChatGroups(); _checkChatJoinParam(); }
  renderAll();
}
...
async function _checkChatJoinParam() {
  ...
  const uid_user = window._currentUid;
  if (!db || !uid_user) {
    // Guardar para depois do auth
    sessionStorage.setItem('lumen_pending_join', joinId);
    return;
  }
  await _joinGroup(joinId);
}
...
// --- Abrir sala de chat ---
async function openChatRoom(groupId, groupName) {
  _currentGroupId = groupId;
  document.getElementById('chatGroupsView').style.display = 'none';
  document.getElementById('chatRoomView').style.display = 'flex';
  document.getElementById('chatRoomName').textContent = groupName;
  document.getElementById('chatRoomMembers').textContent = '';
  document.getElementById('chatMessages').innerHTML = '<div class="chat-loading">A carregar mensagens…</div>';

  // Carregar info do grupo (membros)
  const db = window._firestoreDb;
  if (db) {
    try {
      ...
      if (snap.exists()) {
        const g = snap.data();
        const count = g.members ? g.members.length : 1;
        document.getElementById('chatRoomMembers').textContent = `${count} membro${count !== 1 ? 's' : ''}`;
      }
    } catch(e) {}
  }

  _listenChatMessages(groupId);
}
...
async function _listenChatMessages(groupId) {
  if (_chatUnsubscribe) { _chatUnsubscribe(); _chatUnsubscribe = null; }
  const db = window._firestoreDb;
  if (!db) return;
  try {
    ...
    console.error('[Chat] listen erro:', e);
    document.getElementById('chatMessages').innerHTML = '<div class="chat-loading">Erro ao carregar mensagens.</div>';
  }
}
...
function _renderMessages(msgs) {
  ...
  if (!msgs.length) {
    container.innerHTML = '<div class="chat-loading" style="opacity:0.5">Nenhuma mensagem ainda. Começa a conversa ✦</div>';
    return;
  }
  ...
  let lastDate = '';
  msgs.forEach(m => {
    const isMine = m.uid === uid_user;
    const ts = m.ts ? (m.ts.toDate ? m.ts.toDate() : new Date(m.ts)) : new Date();
    const dateStr = ts.toLocaleDateString('pt-PT', { day: '2-digit', month: 'short' });
    ...
    const avatar = m.photo
      ? `<img src="${m.photo}" class="chat-avatar-img" onerror="this.style.display='none';this.nextSibling.style.display='flex'" /><span class="chat-avatar-initials" style="display:none">${initials}</span>`
    ...
    html += `
      <div class="chat-msg ${isMine ? 'mine' : 'theirs'}">
        ...
        <div class="chat-bubble-wrap">
          ${!isMine ? `<div class="chat-sender">${m.name || 'Utilizador'}</div>` : ''}
          <div class="chat-bubble">${_escapeHtml(m.text)}</div>
...
async function sendChatMessage() {
  ...
  if (!text || !_currentGroupId) return;
  const db = window._firestoreDb;
  const uid_user = window._currentUid;
  if (!db || !uid_user) { toast('Sem ligação.'); return; }
  ...
  try {
    ...
    await addDoc(collection(db, 'groups', _currentGroupId, 'messages'), {
      uid: uid_user,
      name: user.name || state.settings.userName || 'Utilizador',
      photo: user.photo || state.settings.profilePic || '',
      text,
      ts: serverTimestamp(),
    });
  } catch(e) {
...
function shareChatInvite() {
  if (_currentGroupId) shareChatInviteById(_currentGroupId, document.getElementById('chatRoomName').textContent);
}

function shareChatInviteById(groupId, groupName) {
  const base = window.location.origin + window.location.pathname.replace(/\/[^/]*$/, '/index.html');
  const link = `${base}?join=${groupId}`;
  if (navigator.share) {
    navigator.share({ title: `Junta-te ao grupo "${groupName}" no Lúmen`, url: link }).catch(() => {});
  } else {
    navigator.clipboard.writeText(link).then(() => toast('Link copiado! ◫')).catch(() => {
      prompt('Copia este link de convite:', link);
    });
  }
}
...
async function deleteChatGroup(groupId) {
  if (!confirm('Apagar este grupo? Esta ação é irreversível.')) return;
  const db = window._firestoreDb;
  if (!db) return;
  try {
    const { doc, collection, getDocs, deleteDoc } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js');
    // Apagar mensagens primeiro
    const msgs = await getDocs(collection(db, 'groups', groupId, 'messages'));
    await Promise.all(msgs.docs.map(d => deleteDoc(doc(db, 'groups', groupId, 'messages', d.id))));
    await deleteDoc(doc(db, 'groups', groupId));
    toast('Grupo apagado.');
    renderChatGroups();
  } catch(e) {
    console.error('[Chat] delete erro:', e);
    toast('Erro ao apagar grupo.');
  }
}
...
async function leaveChatGroup(groupId) {
  ...
  if (!db || !uid_user) return;
  try {
    const { doc, updateDoc, arrayRemove } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js');
    await updateDoc(doc(db, 'groups', groupId), { members: arrayRemove(uid_user) });
    toast('Saíste do grupo.');
    renderChatGroups();
  } catch(e) {
    console.error('[Chat] leave erro:', e);
